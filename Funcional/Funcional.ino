...
uint16_t getColorForAngle(float angle) {
  ...
  // Dividir el círculo en 20 segmentos de 18 grados cada uno y asignar colores
...
void loop() {
  ...
  // Mostrar el color del eje con el mayor valor absoluto
  if (abs(angleX) > abs(angleY)) {
    tft.fillScreen(colorX);  // Mostrar el color basado en el ángulo del eje X
  } else {
    tft.fillScreen(colorY);  // Mostrar el color basado en el ángulo del eje Y
  }
